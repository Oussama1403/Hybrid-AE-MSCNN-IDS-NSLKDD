...
A hybrid deep learning architecture combining an **Autoencoder (AE)** with a **Multi-Scale 1D CNN (MSCNN)** and a **learned MLP calibrator** for detecting zero-day (unknown) network intrusions. The system is evaluated on the **NSL-KDD** benchmark, where 17 attack types appear **exclusively in the test set** — simulating a realistic zero-day detection scenario.
...
- [Architecture](#architecture)
  - [1. Autoencoder Branch (Unsupervised Anomaly Detection)](#1-autoencoder-branch-unsupervised-anomaly-detection)
  - [2. Multi-Scale 1D CNN Branch (Supervised Classification)](#2-multi-scale-1d-cnn-branch-supervised-classification)
  - [3. MLP Calibrator (Learned Fusion)](#3-mlp-calibrator-learned-fusion)
- [Feature Pipeline](#feature-pipeline)
- [Data Preprocessing & Balancing](#data-preprocessing--balancing)
- [Dataset Shift & Zero-Day Analysis](#dataset-shift--zero-day-analysis)
- [Experimental Results](#experimental-results)
  - [Overall Performance](#overall-performance)
  - [Per-Class Breakdown](#per-class-breakdown)
  - [Unknown Attack Recall by Type](#unknown-attack-recall-by-type)
...
Trained **exclusively on normal (benign) traffic** to learn a compact representation of legitimate behavior.
...
- **Anomaly Scoring:** Reconstruction error is converted to a **q-score** — the percentile of the per-sample error relative to the normal validation distribution
...
A discriminative classifier that processes a **308-dimensional fused input** (see [Feature Pipeline](#feature-pipeline)) through parallel convolutions at three scales.
...
A lightweight neural gate that fuses CNN and AE signals into a unified decision.
...
|---|---|---|
| `cnn_attack_prob` | CNN | Temperature-scaled softmax probability of attack class |
| `ae_q_score` | AE | Percentile of reconstruction error (vs. normal validation) |
...
| `recon_weighted_mse_z` | AE | Feature-weighted MSE (W = 1/σ²) |
| Residual PCA | AE | Up to 16 PCA components on squared residuals (optional) |

- Trained with **weighted BCE loss** (class-balanced)
- **Constraint-driven optimization:** Selects decision threshold maximizing attack recall while keeping FPR ≤ 0.12
...
Raw Input (122-dim after OHE + scaling)
        ...
        │       └── residual_vector = |X - X_recon|²  (122-dim)
        │
...
- **Class imbalance:** Mitigated via **Borderline-SMOTE** (borderline-1 variant) applied to the CNN training split — generates synthetic samples near the decision boundary
...
## Dataset Shift & Zero-Day Analysis

The `nslkdd_dataset_report` module provides a comprehensive analysis of the NSL-KDD dataset, highlighting the distributional shift between training and test sets.
...
- **Zero-day samples:** 3,750 (16.6% of test set)
...
The hybrid architecture **neutralized 13 of 17** unknown attack types, more than doubling zero-day detection compared to the standalone CNN baseline.
...
The hybrid system trades a modest increase in FPR (3.3% → 11.1%) for a dramatic improvement in unknown attack recall (35.9% → 83.5%).
...
├── nslkdd_dataset_report.py               # Dataset analysis & visualization script
...
│   └── kaggle/working/training_runs/      # Per-run artifacts
...
The NSL-KDD dataset files are included in the `NSL-KDD/` directory. Alternatively, download from [Kaggle](https://www.kaggle.com/datasets/hassan06/nslkdd).

### 2. Run the Pipeline

The main pipeline is in `organized-ae-cnn-main-pipeline.ipynb`. It supports two evaluation modes:

- **Strict split** (default): Train on KDDTrain+, test on KDDTest+ (zero-day detection scenario)
- **Merged split:** 80/20 random train/test split (conventional evaluation)

Key parameters can be configured at the top of the notebook or via CLI argument parser embedded in the notebook.
...
- **Report generation:** Custom pandas + matplotlib pipeline
